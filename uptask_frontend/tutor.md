...
- **Último video completado:** 4 videos en sesión del 13/07 — path aliases (@/) + HeadlessUI/Heroicons
  + NavMenu (Popover/Transition) integrado en AppLayout
- **Próximo video:** siguiente en Sección 29 (verificar por CONTENIDO, no número — ver nota abajo)
- **Pregunta de retención para próxima sesión:** ¿En qué DOS archivos hay que configurar el path
  alias `@/` y por qué en los dos (no alcanza con uno solo)?
- **⚠️ NOTA IMPORTANTE:** Numeración de videos desalineada respecto al roadmap confirmado por captura
  (que decía Sección 29 arranca en 483). No perseguir el número exacto — verificar por CONTENIDO.
- **Sección 27:** ✅ COMPLETA
- **Sección 28:** ✅ COMPLETA — 19/19 videos, CRUD de Proyectos y Tareas + middlewares refactorizados
- **Sección 29:** 🔄 En curso — Tailwind (v3), Router + Layout + Outlet + AppLayout con Logo, NavMenu
  (HeadlessUI Popover), path aliases @/ funcionando. Víctor usa capturas de la app como referencia
  visual (carpeta referencias-ui/, nomenclatura 01_nombre_pantalla.png)
...
### Sesión 10/07/2026 — PRIMER DÍA DE FRONTEND 🚀 — TROUBLESHOOTING SERIO
- Videos: Instalación de Vite + Tailwind + Router (numeración del curso desalineada, ver nota arriba)
- Incidente técnico mayor: Vite 8 (lanzado 12 marzo 2026) reemplazó su bundler a Rolldown (Rust),
  que requiere binarios nativos. Bug conocido de npm (issue #4828, 3 años de antigüedad, 680+
  comentarios) + versión de Node desactualizada (v22.11.0, insuficiente para Vite 8) causaron que
  `npm run dev` fallara reiteradamente.
- El tutor cometió un error inicial: recomendó `ignore-scripts=true` por seguridad (tema legítimo,
  hubo ataques reales a Axios en 2026) sin prever que rompería la instalación de binarios nativos
  de Rolldown. Esto le costó tiempo y frustración justificada a Víctor.
- Víctor cuestionó activamente al tutor pidiendo evidencia (fecha del issue de npm, fuente del bug),
  lo cual llevó a diagnosticar la causa raíz real: Node desactualizado. Buena señal de pensamiento
  crítico — no aceptó explicaciones sin verificar.
- Solución: actualizar Node a v24.18.0 LTS (Windows Installer, no Docker) + reinstalación limpia.
  Funcionó al primer intento después del upgrade.
- Cierre: escribió Router.tsx, AppLayout.tsx (con Outlet), DashboardView.tsx, main.tsx.
  Encontró y corrigió solo el bug de falta de `return` en el componente Router (aunque preguntó
  al tutor para confirmar el diagnóstico).
- Retención: no medida numéricamente esta sesión (fue mayormente troubleshooting), pero buena
  comprensión del patrón Layout/Outlet con ejemplos propios (Netflix, incluyendo el caso de
  layouts anidados).
...
---
*Última actualización: Sesión del 13/07/2026 — Path aliases y HeadlessUI funcionando. NavMenu integrado en AppLayout.*